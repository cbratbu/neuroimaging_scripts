...
# Builds CONN .cfg files for PWA subjects with lesion masks
# Extracts RepetitionTime from JSON sidecar if present, otherwise uses default
...
echo "=== Building CONN cfgs (PWA single-subject) ==="
...
for unit in "${units[@]}"; do
  ...
  if [[ -n "$ses" ]]; then
    ...
    lesion="$(pick_first_existing \
      "${unit}/lesion/${subj}_${ses}_desc-lesion_mask.nii" \
      "${unit}/lesion/${subj}_${ses}_desc-lesion_mask.nii.gz" \
    )" || lesion=""
  else
    anat_candidates+=(
    ...
    lesion="$(pick_first_existing \
      "${unit}/lesion/${subj}_desc-lesion_mask.nii" \
      "${unit}/lesion/${subj}_desc-lesion_mask.nii.gz" \
    )" || lesion=""
  fi

  anat="$(pick_first_existing "${anat_candidates[@]}")" || anat=""

  # Require structural + lesion. Functional is optional.
  missing_required=0
  [[ -z "$anat" ]]   && { echo "[SKIP] ${cfg_id}: missing structural (T1w/FLAIR/T2w)"; missing_required=1; }
  [[ -z "$lesion" ]] && { echo "[SKIP] ${cfg_id}: missing lesion_mask"; missing_required=1; }
  ...
  lesion_base="$(basename "$lesion")"
  lesion_name="${lesion_base%.nii}"
  lesion_name="${lesion_name%.nii.gz}"
